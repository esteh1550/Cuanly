  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans text-gray-900">
      ...
      <header className="bg-white shadow-sm sticky top-0 z-30 px-4 py-3 md:hidden flex justify-between items-center">
        ...
        <div className="text-xs font-medium text-gray-500 bg-gray-100 px-2 py-1 rounded">v1.1</div>
      </header>
      ...
      <div className="md:flex md:min-h-screen">
        ...
        {/* Main Content */}
        <main className="flex-1 md:ml-64 p-4 md:p-8 max-w-4xl mx-auto w-full mb-16 md:mb-0">
          {children}
        </main>
      </div>

      {/* Mobile Bottom Navigation */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-4 py-2 z-40 flex justify-between items-center pb-safe shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
        {navItems.map((item) => {
          const Icon = item.icon;
          const isActive = currentView === item.id;
          return (
